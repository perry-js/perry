...
const getStyleString = (size: number) =>
  `width: ${size}px !important; height: ${size}px !important`;
const getViewBox = (size: number) => `0 0 ${size} ${size}`;

interface IBugIconProps {
  size?: number;
  color?: string;
}

const BugIcon: FunctionalComponent<IBugIconProps> = ({
  size = 24,
  color = '#333',
}) => (
  <svg style={getStyleString(size)} viewBox={getViewBox(size)}>
    <path
      fill={color}
